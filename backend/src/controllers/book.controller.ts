...
export class BookController {
  public async getBooksPreview(request: Request, response: Response) {
    try {
      const filtros = {
        ...
        genero: request.query.genero, // mantiene el parámetro
      };
  ...
  public async getBook(request: Request, response: Response) {
    try {
      ...
      if (isNaN(id)) return response.status(400).json("ID inválido");

      const book = await bookService.findBook(id);
      if (!book) return response.status(404).json("Libro no encontrado");
